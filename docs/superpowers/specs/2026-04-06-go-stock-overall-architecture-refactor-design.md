...
这是最后迁的原因是其跨层依赖最多，必须建立在前面链路稳定之后。
...
- 默认测试门禁持续通过。
- 对应 `external` 测试可验证真实数据源链路。
- 关键日志能将失败定位到 `source / service / bridge` 其中一层。
- 第一阶段结束后，不再往 [backend/data](D:/codex_work/go-stock/backend/data) 新增同类功能实现。
