...
func TestAnalyzeSentiment(t *testing.T) {
	...
	text := "新能源车销量继续增长，行业情绪显著改善，产业链公司订单回暖。"
	requireNotBlank(t, "sentiment source text", text)
	...
	requirePositiveLen(t, "split words", len(words))
	t.Logf("分词数量=%d", len(words))
	result, frequencies := AnalyzeSentimentWithFreqWeight(text)
	// 过滤标点符号和分隔符
	cleanFrequencies := FilterPunctuationAndSeparators(frequencies)
	if strings.TrimSpace(result.Description) == "" {
		t.Fatalf("expected sentiment description, got %#v", result)
	}
	if result.Score == 0 && result.PositiveCount == 0 && result.NegativeCount == 0 && len(cleanFrequencies) == 0 {
		t.Fatalf("expected sentiment analysis to produce score or token stats, got result=%#v frequencies=%v", result, cleanFrequencies)
	}
	// 输出结果
	t.Logf("情感分析结果: %s (得分: %.2f, 正面词:%d, 负面词:%d)\n 词频统计结果: %v",
		result.Description,
		result.Score,
